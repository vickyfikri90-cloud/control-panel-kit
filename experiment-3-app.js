window.initExperiment3 = function initExperiment3() {
  const preview = document.querySelector('[data-experiment-preview="3"]');
  const panelRoot = document.querySelector('[data-experiment-panel="3"]');
  ...
  const controls = {
    count: document.getElementById('exp3-count'),
    radius: document.getElementById('exp3-radius'),
    rotate: document.getElementById('exp3-rotate'),
    originY: document.getElementById('exp3-origin-y'),
    duration: document.getElementById('exp3-duration'),
    velocity: document.getElementById('exp3-velocity'),
  };

  const easing = window.initCubicBezierInput(document.getElementById('exp3-easing-root'), {
    onChange: applyAll,
  });
  ...
  const colorInputs = [1, 2, 3, 4, 5].map((index) => (
    window.initColorInput(document.getElementById(`exp3-color-${index}-root`), {
      onChange: applyAll,
    })
  ));

  const snippet = window.initSnippetOutput(document.getElementById('exp3-snippet-root'), {
    filename: 'experiment-3.html',
  ...
  window.ExperimentSettings['3'] = {
    collect: collectSettings,
    apply: applySettings,
  };

  const pending = window.__pendingExperimentDefaults?.['3'];
